import { splitCssParams, type ParamColorObject } from "./css"
...
export function parseColorFunction(css: string): ParamColorObject | undefined {
	const params = splitCssParams(css)
	if (params.length !== 1) {
		return undefined
	}
	...
	const p = param as ParamColorObject
	...
	if (p.params.length > 3) {
		if (typeof last === "string" && p.params[p.params.length - 2] === "/") {
			p.opacity = last
			p.params.splice(p.params.length - 2, 1)
		}
	}
...
export function parseHexColor(css: string): ParamColorObject | undefined {
...
export function parseNamedColor(value: string): ParamColorObject | undefined {

...
function createTextDocumentContentProvider(h: ReturnType<typeof createWorkspacesHandler>) {
	...
	return vscode.Disposable.from(
		...
		vscode.commands.registerCommand("twobj.preview", pickType),
	)
	...
	async function pickType() {
		...
		const configPath = srv.getConfigPath()
		const uri = URI.parse(
			"tailwind:" + Utils.joinPath(configPath ? Utils.dirname(configPath) : srv.workspaceFolder, `${type}.twd`),
		)
...
export async function activate(context: vscode.ExtensionContext) {
	...
	console.info(
		`twobj ${intl.formatMessage({ id: "ext.debug-outout.version" })}:`,
		packageInfo.devDependencies["twobj"],
	)
	console.info(
